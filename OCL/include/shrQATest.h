...
inline int __shrQAStart(int argc, char **argv)
{
    ...
    if (bQATest) {
        fprintf(stderr, "&&&& RUNNING %s", &(argv[0][exename_start]));
...
{
    fprintf(stderr, "> exiting in %d seconds: ", seconds);
    fflush(stderr);
    time_t t;
    int count;
...
inline void __shrQAFinish(int argc, const char **argv, int iStatus)
{
    ...
    int exename_start = findExeNameStart(argv[0]);
    if (bQATest) {
        fprintf(stderr, "&&&& %s %s", sStatus[iStatus], &(argv[0][exename_start]));
        for (int i=1; i < argc; i++) fprintf(stderr, " %s", argv[i]);
        fprintf(stderr, "\n");
    } else {
        fprintf(stderr, "[%s] test results...\n%s\n", &(argv[0][exename_start]), sStatus[iStatus]);
    }
    fflush(stderr);
    printf("\n"); fflush(stdout);
    if (bQuitInTime) {
